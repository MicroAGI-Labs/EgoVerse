"""Validate a produced EgoVerse Zarr episode against the format contract.

This is the canonical implementation of the ``validate_episode`` check
described in ``CONTRIBUTING_DATA.md`` §10.1 — run it on every episode before
uploading. Converters (``mcap_to_zarr.py``, ``aria_to_zarr.py``, …) call it at
the end of conversion so a malformed store never reaches S3.
"""
...
def validate_episode(zarr_path: str) -> tuple[list[str], list[str]]:
    """Validate a single ``.zarr`` episode store.

    Returns ``(errors, successes)``. An empty ``errors`` list means the episode
    passes the format contract in ``CONTRIBUTING_DATA.md`` §5.
    """
    errors: list[str] = []
    successes: list[str] = []
    ...
    if meta.get("fps", 0) not in (30, 60):
        errors.append(f"Unexpected fps={meta.get('fps')}. Expected 30 or 60.")
    else:
    ...
    for key in required_poses + optional_poses:
        if key in store:
            ...
            if not np.allclose(norms, 1.0, atol=1e-4):
                ...
                errors.append(
                    f"{key}: {len(bad)} frames with non-unit quaternions "
                    f"(e.g. frame {bad[0]}, norm={norms[bad[0]]:.6f})"
                )
            else:
    ...
    for key in annotation_keys:
            ...
            try:
                ...
                for field, expected in (("text", str), ("start_idx", int), ("end_idx", int)):
                    ...
                    if not isinstance(rec[field], expected):
                        raise ValueError(
                            f"field '{field}' is {type(rec[field]).__name__}, "
                            f"expected {expected.__name__}"
                        )
                if not (0 <= rec["start_idx"] <= rec["end_idx"] <= T):
                    raise ValueError(
                        f"index range invalid: start={rec['start_idx']}, "
                        f"end={rec['end_idx']}, T={T}"
                    )
        ...
        if bad:
            errors.append(
                f"{key}: {bad}/{n} annotations malformed "
                f"(e.g. index {first_err[0]}: {first_err[1]})"
            )
        else:
...
def log_validation(zarr_path: str, log: logging.Logger | None = None) -> list[str]:
    """Validate ``zarr_path`` and log each check; return the list of errors."""
    log = log or logger
    errors, successes = validate_episode(str(zarr_path))
    for s in successes:
        log.info("validate OK: %s", s)
    for e in errors:
        log.error("validate ERROR: %s", e)
    if errors:
        log.error("%s: %d validation error(s)", zarr_path, len(errors))
    else:
        log.info("%s: all validation checks passed", zarr_path)
    return errors
